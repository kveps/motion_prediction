"""Heuristic ground-truth-free trajectory generators.

Currently only `ballistic_trajectories` lives here — used by the
visualizer to overlay a constant-velocity baseline alongside the model's
predictions.

Multi-modal decoder anchor endpoints (used by the model itself) moved to
`utils/model/endpoint_anchors.py`. See `get_mode_anchor_endpoints`.
"""
import torch
