...
def translate_parsed_dataset_to_av_center(parsed):
    ...
    # Set AV center
    num_current_states = parsed['state/current/x'].shape[1]
    av_center_x = parsed['state/current/x'][av_idx,
                                            num_current_states - 1]
    av_center_y = parsed['state/current/y'][av_idx,
                                            num_current_states - 1]
    av_center_z = 0.0
    ...
    for key, value in parsed.items():
        if key == 'state/past/x':
            ...
            transformed[key] = tf.subtract(
                parsed['state/current/x'], av_center_x)
        elif key == 'state/current/y':
            transformed[key] = tf.subtract(
                parsed['state/current/y'], av_center_y)
        elif key == 'state/future/x':
            transformed[key] = tf.subtract(
                parsed['state/future/x'], av_center_x)
        elif key == 'state/future/y':
            transformed[key] = tf.subtract(
                parsed['state/future/y'], av_center_y)
        elif key == 'roadgraph_samples/xyz':
            transformed['roadgraph_samples/xyz'] = tf.subtract(
                parsed['roadgraph_samples/xyz'], av_center_xyz)
        elif key == 'traffic_light_state/current/x':
            transformed[key] = tf.subtract(
                parsed['traffic_light_state/current/x'], av_center_x)
        elif key == 'traffic_light_state/current/y':
            transformed[key] = tf.subtract(
                parsed['traffic_light_state/current/y'], av_center_y)
        elif key == 'traffic_light_state/current/z':
            transformed[key] = tf.subtract(
                parsed['traffic_light_state/current/x'], av_center_z)
        elif key == 'traffic_light_state/past/x':
            transformed[key] = tf.subtract(
                parsed['traffic_light_state/past/x'], av_center_x)
        elif key == 'traffic_light_state/past/y':
            transformed[key] = tf.subtract(
                parsed['traffic_light_state/past/y'], av_center_y)
        elif key == 'traffic_light_state/past/z':
            transformed[key] = tf.subtract(
                parsed['traffic_light_state/past/x'], av_center_z)
        else:
